...
public class Figure {
    ...
    private boolean isTouchWall(int direction) {
        for (Block block : figure) {
            if (direction == KeyEvent.VK_LEFT && (block.getX() == 0 || mine[block.getY()][block.getX() - 1] > 0)) return true;
            if (direction == KeyEvent.VK_RIGHT && (block.getX() == FIELD_WIDTH - 1 || mine[block.getY()][block.getX() + 1] > 0))
                return true;
        }
        return false;
    }
